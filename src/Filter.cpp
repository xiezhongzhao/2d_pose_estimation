...
//        std::cout << "alphaD: " << alphaD << std::endl;
    ...
    Eurofilter2D::Eurofilter2D(clock_t t0, vector<cv::Point2f> x0, vector<cv::Point2f> DX0,
                               float minCutoff, float beta, float dCutoff) {
    ...
    vector<cv::Point2f> Eurofilter2D::ExponentialSmoothing(vector<cv::Point2f>& alpha,
                                                           vector<cv::Point2f>& CurrValue,
...
//        std::cout << "alphaVal: " << alphaVal << std::endl;
    ...
    Eurofilter3D::Eurofilter3D(clock_t t0, vector<cv::Point3f> x0, vector<cv::Point3f> DX0,
                               float minCutoff, float beta, float dCutoff) {
    ...
    vector<cv::Point3f> Eurofilter3D::ExponentialSmoothing(vector<cv::Point3f>& alpha,
                                                           vector<cv::Point3f>& CurrValue,
...
//        std::cout << "alphaVal: " << alphaVal << std::endl;
    ...
    float Filter1D(float t, float val, float MinCutoff, float Beta, float DCutoff){
        float DX0 = 0;
        static filter::OneEuroFilter euro(t, val, DX0,
    ...
    vector<cv::Point2f> Filter2D(const vector<cv::Point2f>& keypoints, float MinCutoff, float Beta, float DCutoff){
        vector<cv::Point2f> DX02D(keypoints.size(),cv::Point2f (0,0));
        static filter::Eurofilter2D euro2D(clock(), keypoints, DX02D,
                                           MinCutoff, Beta, DCutoff);
    ...
    vector<cv::Point3f> Filter3D(const vector<cv::Point3f>& keypoints, float MinCutoff, float Beta, float DCutoff){
        vector<cv::Point3f> DX03D(keypoints.size(), cv::Point3f (0,0,0));
        static filter::Eurofilter3D euro3D(clock(), keypoints, DX03D,
                                           MinCutoff, Beta, DCutoff);
