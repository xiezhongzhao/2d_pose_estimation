...
public:
    Eurofilter2D(clock_t t0, vector<cv::Point2f> x0, vector<cv::Point2f> DX0,
                  float MinCutoff =1.700, float Beta = 0.300, float DCutoff = 30.0);
    ...
    vector<cv::Point2f> ExponentialSmoothing(vector<cv::Point2f>& alpha,
...
public:
    Eurofilter3D(clock_t t0, vector<cv::Point3f> x0, vector<cv::Point3f> DX0,
                 float MinCutoff =1.700, float Beta = 0.300, float DCutoff = 30.0);
    ...
    vector<cv::Point3f> ExponentialSmoothing(vector<cv::Point3f>& alpha,
...
vector<cv::Point2f> Filter2D(const vector<cv::Point2f>& keypoints,
                         float MinCutoff =1.700, float Beta = 0.300, float DCutoff = 30.0);

vector<cv::Point3f> Filter3D(const vector<cv::Point3f>& keypoints,
                             float MinCutoff =1.700, float Beta = 0.300, float DCutoff = 30.0);
};
