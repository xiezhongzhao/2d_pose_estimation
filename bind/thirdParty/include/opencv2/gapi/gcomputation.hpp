 ...
 * executed. The below example.py expresses calculation of Sobel operator
 ...
 *
 * In the above example.py, sobelEdge expects one Mat on input and
 ...
 * cv::GComputation::apply(). For example.py, if a graph is defined on
